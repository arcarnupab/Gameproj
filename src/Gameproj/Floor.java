...
import Gameproj.AbstractCharacter.Move;
import java.util.*;
...
public class Floor {
    ...
    private boolean isGameOver = false;  ///เช็คแพ้เกม

    public Floor(int width, int height, int nrOfEnemies) {
	this.width = width;  //กว้างแมพ
	this.height = height;  //สูงแมพ
	this.tiles = new FloorTile[height][width];  // title แมพ
	placeBreakable();  ///กำแพงระเบิด
	placeUnbreakableAndGrass();  ///กำแพงที่ระเบิดไม่ได้
	spawnEnemies(nrOfEnemies);  ///pass จำนวน enemy to spawnEnemies
    }
